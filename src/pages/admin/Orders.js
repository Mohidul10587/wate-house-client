...
const Orders = () => {
  ...
  const { data: orderedVouchers, isLoading, refetch } = useQuery('orderedVouchers', () => fetch('http://localhost:5000/orderedVoucher', {
    method: 'GET',
    headers: {
  ...
  const deleteVoucher = id => {
    ...
    fetch(`http://localhost:5000/orderedVoucher/${id}`, {
      method: 'DELETE',
    }).then(res => res.json())
      .then(data => {
